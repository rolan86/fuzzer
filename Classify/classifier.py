...
def get_clean_data(data):
    ...
    return (data.drop(['Survived', 'Name'], axis=1),
...
def get_survival(training_data, query):
    """ Get the survival of the member """

    result_dict = {}
    for row in get_fuzz_name(training_data[2], query).index.tolist():
        survival = train_classifier_up(
            training_data, pd.DataFrame(training_data[0].iloc[row]).T
        )
        survival = [
            'Deceased' if info == 0 else 'Survived'
            for info in survival
        ]
        result_dict[training_data[2].iloc[row].Name] = {'survival': survival}
    return result_dict
